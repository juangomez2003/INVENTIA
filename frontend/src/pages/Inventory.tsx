      ...
      <div className="animate-fade-up delay-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(90px, 1fr))', gap: 10 }}>
        {Object.entries(stats).map(([key, value]) => (
          <div key={key} className="card" style={{ padding: '16px 14px', textAlign: 'center', borderRadius: 12 }}>
            <p style={{ fontSize: 26, fontWeight: 700, color: statColors[key], letterSpacing: '-0.02em' }}>{value}</p>
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          ...
          <div
            ...
            style={{ position: 'relative', borderRadius: 20, padding: 28, width: '100%', maxWidth: 500 }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <h3 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-1)', letterSpacing: '-0.02em' }}>Agregar producto</h3>
              <button
                onClick={() => setShowAddModal(false)}
                style={{ color: 'var(--text-3)', background: 'var(--surface-hover)', border: 'none', cursor: 'pointer', padding: 8, borderRadius: 8, display: 'flex' }}
              >
                <X style={{ width: 17, height: 17 }} />
              </button>
            </div>

            <form style={{ display: 'flex', flexDirection: 'column', gap: 14 }} onSubmit={(e) => { e.preventDefault(); setShowAddModal(false); }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div>
                  <label style={labelStyle}>Nombre</label>
                  <input style={inputStyle} placeholder="Ej: Pollo" onFocus={focusInput} onBlur={blurInput} />
                </div>
                <div>
                  <label style={labelStyle}>Categoría</label>
                  <select style={{ ...inputStyle, appearance: 'none' }} onFocus={focusInput} onBlur={blurInput}>
                    <option>Carnes</option><option>Verduras</option><option>Frutas</option>
                    <option>Granos</option><option>Lácteos</option><option>Mariscos</option><option>Aceites</option>
                  </select>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
                <div>
                  ...
                  <input type="number" style={inputStyle} placeholder="0" onFocus={focusInput} onBlur={blurInput} />
                </div>
                <div>
                  <label style={labelStyle}>Unidad</label>
                  <input style={inputStyle} placeholder="kg, L, pz" onFocus={focusInput} onBlur={blurInput} />
                </div>
                <div>
                  <label style={labelStyle}>Precio/U</label>
                  <input type="number" step="0.01" style={inputStyle} placeholder="0.00" onFocus={focusInput} onBlur={blurInput} />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div>
                  ...
                  <input type="number" style={inputStyle} placeholder="Umbral mínimo" onFocus={focusInput} onBlur={blurInput} />
                </div>
                <div>
                  <label style={labelStyle}>Capacidad máx.</label>
                  <input type="number" style={inputStyle} placeholder="Capacidad máxima" onFocus={focusInput} onBlur={blurInput} />
                </div>
              </div>

              <div>
                <label style={labelStyle}>Proveedor</label>
                <input style={inputStyle} placeholder="Nombre del proveedor" onFocus={focusInput} onBlur={blurInput} />
              </div>
